    ...
    <form onSubmit={handleSubmit} noValidate className="space-y-4">
      ...
      <fieldset className={cn(panelSurface(), "space-y-4 p-3")}>
        ...
        <div className="grid grid-cols-[1fr_auto] gap-4">
          ...
          {/* Deliberately no fixed width: the enclosing grid's second track is
              `auto`, so this column sizes to its own content — which is the
              label, not the input (Input is `w-full`). A hardcoded `w-24`
              (96px) lived here and silently broke when #54's label unification
              made OptionalLabel an `inline-flex` that cannot wrap: "PORT
              (optional)" measures ~124px and spilled ~15px past the panel
              border. Letting the track do the measuring cannot drift that way
              again, whatever the label says or which font is loaded. */}
          <div className="space-y-1">
